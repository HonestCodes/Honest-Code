...
namespace GSwap.Services
{
    public class FileService : IFileService
    {
        ...
        //    = _siteConfigService.AwsBucket;
        //private string keyName = _siteConfigService.;
        ...
        public List<string> UploadFiles(HttpFileCollection hfc)
        {
            ...
            {
                HttpPostedFile hpf = hfc[i]; //get this guys stream 
        ...
        public Models.Domain.Files.File CookFileInfo(int id)
        {
            Models.Domain.Files.File myFile = null;

            Action<IDataReader, short> singleRecMapper = delegate (IDataReader reader, short set)
            {
                myFile = MapFile(reader);
            };
            ...
            _dataProvider.ExecuteCmd("dbo.Files_SelectById2", inputParamDelegate, singleRecMapper);
            return myFile;
        }
        ...
        public List<Models.Domain.Files.File> GetFilesForLoggedInUser(int userId)
        {
            List<Models.Domain.Files.File> myFile = null;
            ...
            Action<IDataReader, short> singleRecMapper = delegate (IDataReader reader, short set)
            {
                ...
                if (myFile == null)
                {
                    myFile = new List<Models.Domain.Files.File>();
                }
               
                myFile.Add(MapFile(reader));
            };
            ...
            _dataProvider.ExecuteCmd("dbo.Files_SelectByUserId", inputParamDelegate, singleRecMapper);
            return myFile;
        }


        private static Models.Domain.Files.File MapFile(IDataReader reader)
        {
            Models.Domain.Files.File myFile = new Models.Domain.Files.File();
            int startingIndex = 0;

            myFile.Id = reader.GetSafeInt32(startingIndex++);
            myFile.UserId = reader.GetSafeInt32(startingIndex++);
            myFile.FileName = reader.GetSafeString(startingIndex++);
            myFile.FileTypeId = reader.GetSafeInt32(startingIndex++);
            myFile.DateAdded = reader.GetSafeDateTime(startingIndex++);
            myFile.DateModified = reader.GetSafeDateTime(startingIndex++);
